...
function Test-KeywordIsNegationPhrase {
    ...
    $pattern = "(不使用|不指定|不需要|不要|不用|无需|避免|排除|without\b|no\b|not\s+using\b|do\s+not\s+use\b|don't\s+use\b|not\b)"
...
function Test-KeywordMatchInNegatedScope {
    ...
    $negationPattern = "(不使用|不指定|不需要|不要|不用|无需|避免|排除|without\b|no\b|not\s+using\b|do\s+not\s+use\b|don't\s+use\b|not\b)"
